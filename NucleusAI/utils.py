...
def run_local(config, destination='training'):
    ...
    if destination == 'training':
        ...
        if twoDim:
            print('2-DIMENSION')
            extract_features_2D.main(inputfile, maskfile, outputfile)
        else:
            print('3-DIMENSION')
            extract_features_3D.main(inputfile, maskfile, outputfile, nrofthreads)
        ...
        op_image_path = os.path.join(config['outputDir'], 'ValidationOutput')
        ...
        if twoDim:
            print("Validate-2D")
            scores_dict = validate2d(model_path, inputDir, maskDir, outputDir, extension, multichannel, memory_usage)

            return scores_dict
        else:
            print("Validate-3D")
            scores_dict = validate3d(model_path, inputDir, maskDir, outputDir, extension, memory_usage)

            return scores_dict
            
    else:
        inputDir = config['inputDir']
        opDir = config['outputDir']
        modelDir = config['modelDir']
        twoDim = config['twoDim']
        extension = config['extension']
        multichannel = config['multichannel']

        op_image_path = os.path.join(opDir, 'SegmentedImages')
        os.makedirs(op_image_path, exist_ok=True)
        outputDir = op_image_path
